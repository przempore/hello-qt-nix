{
  ...
  outputs = { self, nixpkgs, devenv, cvb, ... } @ inputs:
  let
  ...
  in {
    packages.x86_64-linux.default = pkgs.stdenv.mkDerivation {
      name = "cvb-qt-dev";
      src = ./.;
      nativeBuildInputs = [ pkgs.cmake pkgs.ninja pkgs.gdb pkgs.gcc ];
      buildInputs = [ pkgs.zlib pkgs.glib pkgs.libglibutil pkgs.clang-tools pkgs.boost llvm.lldb llvm.clang pkgs.tree ];
      shellHook = ''
